...
5. **素材** —— 仅封面经图片 provider 生成并填充 `asset_path`；正文插图不调用生图 provider，`asset_path` 恒为空：设计稿主体经 `optimize_prompt` 优化为五要素提示词（主体/风格/构图/色调/约束），一律以 `:::figure` 文本占位块呈现在正文——成品正文无 `<img>` 标签。
...
- 成品必有占位符：主题启用 `figure` 组件时，正文插图（含设计稿主体）一律经 `optimize_prompt` 优化为五要素提示词，以 `:::figure` 占位块呈现在正文（虚线卡片样式，六主题内置），成品正文无 `<img>` 标签；主题未启用 `figure` 时才允许人工编排以 `![purpose](asset)` 插真图。`--apply` 先清后插，幂等可重跑。
